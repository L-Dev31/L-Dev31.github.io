  ...
  file.text()
    .then((raw) => {
      ...
      entries.forEach((item, idx) => {
        ...
        // Accept either a single 'message' string or, for sequenced groups, a
        // 'messages' array (exported format uses `messages` for sequenced entries).
        const hasSequencedGroup = Array.isArray(item.sequencedGroup) && item.sequencedGroup.length > 1;
        const hasMessageString = typeof item.message === 'string';
        const hasMessagesArray = Array.isArray(item.messages) && item.messages.every(m => typeof m === 'string');
        if (!hasMessageString && !(hasSequencedGroup && hasMessagesArray)) {
          throw new Error(`Entry ${id} is missing a string "message" field.`);
        }
        seen.add(id);
        if (hasSequencedGroup) {
          // For sequenced groups, accept either a single concatenated message
          // in `message` or per-target segments in `messages`.
          if (hasMessagesArray) {
            messages.set(id, {
              isSequenced: true,
              indices: item.sequencedGroup,
              message: item.messages // array of strings
            });
          } else {
            messages.set(id, {
              isSequenced: true,
              indices: item.sequencedGroup,
              message: item.message // single string to be split
            });
          }
        } else {
          messages.set(id, {
      ...
      // Allow partial imports: only apply messages that were provided.
      // Collect missing ids and report them once to avoid spamming the console
      // when many ids are not present in the imported JSON.
      const missingIds = [];
      ...
      if (missingIds.length) {
        ...
        console.warn(`Imported JSON: no message provided for entry ids: ${uniq.join(', ')}; preserving originals.`);
        // show a brief UI notice (non-blocking)
        try {
          showMessage(`Imported JSON: missing messages for ${uniq.length} entries (see console).`, 'warning');
        } catch (e) {
          // If UI isn't ready, fall back to console only.
        }
      }
      
      const appliedSequenced = [];
      state.entries.forEach((entry, index) => {
        const data = messages.get(entry.index);
        if (!data) return;
        ...
        if (data.isSequenced) {
          ...
          if (targets.length) {
            // If the imported data provides per-target messages (array), apply
            // them directly. Otherwise, treat `data.message` as a single full
            // text and split it proportionally among targets.
            if (Array.isArray(data.message)) {
              const details = { sourceId: entry.index, targets: [] };
              targets.forEach((targetEntry, idx) => {
                const raw = data.message[idx] ?? '';
                const part = normalizeInput(raw);
                const before = targetEntry.text;
                targetEntry.text = part;
                const encodedLength = encodeBmgString(targetEntry.text, { leadingNull: targetEntry.leadingNull }).length;
                targetEntry.dirty = targetEntry.text !== targetEntry.originalText;
                targetEntry.byteLength = targetEntry.dirty ? encodedLength : targetEntry.originalBytes.length;
                details.targets.push({ id: targetEntry.index, before, after: targetEntry.text });
              });
              appliedSequenced.push(details);
            } else {
              const details = { sourceId: entry.index, targets: [] };
            ...
            scrollingGroup.forEach((variantId, index) => {
              ...
              if (entry && variants[index] !== undefined) {
                ...
                if (Array.isArray(item.sequencedGroup) && item.sequencedGroup.length > 1) {
                  ...
                  const targets = ids.map(id => state.midStrings[id]).filter(Boolean);
                  if (targets.length) {
                    const origVis = targets.map(t => Math.max(1, countVisibleCharacters(t.originalText)));
                    const sumOrig = origVis.reduce((a, b) => a + b, 0) || targets.length;
                    const fullVis = countVisibleCharacters(normalizedFull);
      ...
      // Report what we applied for sequenced groups to help debugging.
      // Always refresh UI after applying changes.
      updateCalculatedOffsets();
      renderEntries();
      updateSaveButton();

      if (appliedSequenced.length) {
        console.group('Imported sequenced updates');
        appliedSequenced.forEach((d) => {
          console.log(`source ${d.sourceId}:`);
          d.targets.forEach(t => console.log(`  id ${t.id}: before='${t.before}' → after='${t.after}'`));
        });
        console.groupEnd();
        showMessage(`Imported messages from JSON. Applied ${appliedSequenced.length} sequenced groups. See console for details.`, 'success');
      } else {
